...
# Permet d'afficher les differents menus.
# Chaque méthode récupère le choix de l'utilisateur.
class Menus:
    ...
    def display_player_menu(self):
        ...
        choix_utilisateur = basic_functions.entry_user_int(
            "Que voulez vous faire ?\n",
            1,
            3
        )
    ...
    def display_tournament_loading_menu(
        self,
        saved_tournaments,
        db_tournaments,
        saved_players,
        ...
        if len(tournaments_playing) == 0:
            ...
            for tournament in tournaments_playing:
                print_(
                    "\nEntrez la commande : (" +
                    str(i) +
                    ") pour reprendre le tournois : " +
                    tournament.name
                )
                i += 1
                tournament.display_tournament_basic()
            choix_utilisateur = entry_user_int(
                "Quel tournoi voulez vous reprendre ?",
                1,
                len(tournaments_playing)
            ) - 1

            print_("Reprise du tournois suivant :")
            tournaments_playing[choix_utilisateur].display_tournament()
            tournaments_playing[choix_utilisateur].process_tournament(
                saved_tournaments,
                db_tournaments,
                saved_players,
                db_players
