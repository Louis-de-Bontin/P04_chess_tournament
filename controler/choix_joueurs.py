...
def choix2(menu, saved_players, db_players): 
    ...
    if reponse_utilisateur == 1:
        ...
        saved_players = gestion_joueurs.create_player(saved_players, db_players)
        for player in saved_players:
            player.display()

    if reponse_utilisateur == 2:
        # Affichage des joueurs existant (ou d'un message d'information si il n'en n'existe aucun), et selection de l'utilisateur
    ...
    if reponse_utilisateur == 3:
        # Affichage des joueurs existant (ou d'un message d'information si il n'en n'existe aucun)
        display_players_header()
        if len(saved_players) > 0:
            for player in saved_players:
                # print(player.first_name + "loop")
                # print(player)
                player.display()
        else:
